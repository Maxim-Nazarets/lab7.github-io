...
function playerShootsGunman() {
    ...
    // Генеруємо випадкове число між 0.50 і 0.80


    document.querySelector('.time-panel__gunman').textContent = (reactTime / 1000).toFixed(2);
...
function gunmanShootsPlayer() {
    ...
    const randomReaction = (Math.random() * (0.80 - 0.50) + 0.50).toFixed(2);

// Виводимо його замість реального часу
    document.querySelector('.time-panel__you').textContent = randomReaction;
    messageBox.className = 'message message--dead';
